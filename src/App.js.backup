                            ...
                            {getTableDates(showPastDates, allDates).map((date) => (
                              <td key={date.date} style={{ minWidth: "80px" }}>
                                <AttendanceToggle
                                  value={fussAttendance[date.date]?.[name] || "Ei"}
                                  onChange={(val) => {
                                    // Create a shallow copy of the existing date data or an empty object
                                    const dateData = fussAttendance[date.date] ? 
                                      {...fussAttendance[date.date]} : {};
                                    
                                    // Update just this player's attendance value
                                    dateData[name] = val;
                                    
                                    // Create a new attendance object with the updated date
                                    const updatedAttendance = {...fussAttendance};
                                    updatedAttendance[date.date] = dateData;
                                    
                                    // Update local state
                                    setFussAttendance(updatedAttendance);
                                    
                                    // Update just this date node in Firebase
                                    set(
                                      databaseRef(db, `fussAttendance/${date.date}`),
                                      dateData
                                    );
