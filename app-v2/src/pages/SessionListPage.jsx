...
function SessionCardCoach({ instId, inst, def, attendance, rosters, isActive, onClick }) {
    ...
    return (
        <div
            onClick={onClick}
            style={{
                border: "1px solid " + (isActive ? "var(--color-success)" : "#ccc"),
        ...
        >
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px" }}>
                ...
                {isActive && (
                    <div style={{ background: "var(--color-success)", color: "white", padding: "2px 8px", borderRadius: "12px", fontSize: "12px", fontWeight: "bold" }}>
                        AKTIIVNE
                    </div>
                )}
            </div>
            <div style={{ color: "var(--color-text-secondary)", marginBottom: "var(--spacing-xs)" }}>{formatEstonianDate(inst.date)}</div>
...
// ─── Parent Session Card ────────────────────────────────
function SessionCardParent({ instId, inst, def, attendance, rosters, players, sessionMessages, sessionFeedback, childName, playerId, nowMs, onPreStatus, msg, isExtraSession = false, myExtraRequest = null, onCancelExtraRequest, onRequestExtra }) {
    ...
    return (
        <div style={{
            ...
            >
                <div>
                    <div style={{ fontWeight: "var(--font-weight-bold)", fontSize: "15px", color: "var(--color-primary-dark)", marginBottom: "var(--spacing-xs)" }}>{childName}</div>
                    <div style={{ fontWeight: "var(--font-weight-bold)", fontSize: "16px" }}>{timeDisplay}</div>
                    <div style={{ fontSize: "12px", color: "var(--color-text-secondary)", marginTop: "var(--spacing-sm)" }}>
                        {formatEstonianDate(inst.date)}
                    </div>
                    <div style={{ fontSize: "12px", color: "var(--color-text-muted)", textTransform: "capitalize", marginTop: "var(--spacing-sm)" }}>
                        {sport}
                    </div>
                </div>
            </div>
                ...
                <>
                        ...
                        <SectionBlock>
                            {isLocked ? (
                                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                                    <StatusText type={getStatusType(preStatus)}>
                                ...
                                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                                    <StatusText type={getStatusType("kinnitatud")}>✅ {PRESTATUS_LABELS.kinnitatud}</StatusText>
                                    <SecondaryButton onClick={e => { e.stopPropagation(); onPreStatus(instId, playerId, null) }}>
                                        Tühista
                                    </SecondaryButton>
                                </div>
                            ) : preStatus === "eiOsale" ? (
                                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                                    <StatusText type={getStatusType("eiOsale")}>❌ {PRESTATUS_LABELS.eiOsale}</StatusText>
                                    <PrimaryButton onClick={e => { e.stopPropagation(); onPreStatus(instId, playerId, "kinnitatud") }}
                                        disabled={isFull}>
                                        Muuda
                                    </PrimaryButton>
                    ...
                    {sessionStarted && realStatus && (
                        <div style={{ marginBottom: "12px", fontSize: "14px" }}>
                            Kohalolek: <StatusText type={getStatusType(realStatus)}>
                                ...
                                <div style={{ marginBottom: "var(--spacing-sm)" }}>
                                    ...
                                    {feedbackVisible ? (
                                        <span style={{ fontSize: "13px" }}>
                                            ...
                                            {coachFb.note && <span style={{ marginLeft: "var(--spacing-sm)", fontStyle: "italic", color: "#666" }}>"{coachFb.note}"</span>}
                                        </span>
                                    ) : (
                                        <span style={{ fontSize: "13px", color: "var(--color-warning)", fontStyle: "italic" }}>Treeneri tagasiside on varsti saadaval</span>
                                    )}
                                </div>
                            )}
        ...
        <div style={{
            ...
            {isExpanded && (
                <>
                    ...
                    {canFeedback && coachFb && (
                        <SectionBlock>
                            <div style={{ fontSize: "14px", fontWeight: "bold", marginBottom: "var(--spacing-xs)" }}>Treeneri tagasiside</div>
                            {feedbackVisible ? (
                                <div style={{ fontSize: "13px" }}>
                                    {coachFeedbackSummary?.expanded || coachFb.effort}
                                    {coachFb.note && <div style={{ marginTop: "var(--spacing-xs)", fontStyle: "italic", color: "#666" }}>"{coachFb.note}"</div>}
                                </div>
                            ) : (
                                <div style={{ fontSize: "13px", color: "var(--color-warning)", fontStyle: "italic" }}>Treeneri tagasiside on varsti saadaval</div>
                            )}
                        </SectionBlock>
                    )}
                    ...
                    {canFeedback && (() => {
                        if (isExpired && !hasFeedback) {
                            return <div style={{ fontSize: "13px", color: "var(--color-text-muted)", marginBottom: "var(--spacing-sm)" }}>Tagasiside aeg lõppenud</div>
                        }
                        if ((isExpired && hasFeedback) || (hasFeedback && !isEditingFb)) {
                            ...
                            return (
                                <SectionBlock>
                                    ...
                                    <div style={{ fontSize: "13px", marginBottom: "var(--spacing-xs)" }}>Pingutus: {effortItem?.emoji} {effortItem?.label}</div>
                                    <div style={{ fontSize: "13px", marginBottom: "var(--spacing-xs)" }}>Treener: {engItem?.emoji} {engItem?.label}</div>
                                    {existingFb.note && <div style={{ fontSize: "13px", color: "var(--color-text-secondary)", fontStyle: "italic", marginBottom: "var(--spacing-xs)" }}>{existingFb.note}</div>}
                                    {!isExpired && (
                                    ...
                                    {feedbackSaved[key] && <span style={{ marginLeft: "var(--spacing-sm)", color: "var(--color-success)", fontWeight: "bold", fontSize: "13px" }}>Salvestatud ✓</span>}
                                </SectionBlock>
                            )
                        }
                        ...
                        return (
                            <SectionBlock>
                                <div style={{ fontSize: "14px", fontWeight: "bold", marginBottom: "var(--spacing-sm)" }}>Minu tagasiside</div>
                                ...
                                <input type="text" value={local.note}
                                    ...
                                    style={{ width: "100%", padding: "var(--spacing-sm)", borderRadius: "6px", border: "1px solid #ccc", marginBottom: "var(--spacing-sm)", boxSizing: "border-box", fontSize: "13px" }}
                                />
                                <div style={{ display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap" }}>
                                    ...
                                    {feedbackSaved[key] && <span style={{ color: "var(--color-success)", fontWeight: "bold", fontSize: "13px" }}>Salvestatud ✓</span>}
                                </div>
                            </SectionBlock>
                        )
                    })()}
...
export default function SessionListPage() {
    ...
    function validatePreStatusChange({
        instId,
        playerId,
        newStatus,
        currentInst,
        ...
        const sessionStartMs = new Date(combineDateAndTime(currentInst.date, startTime)).getTime()
        const capacity = currentInst.capacity || def?.capacity || 0

        // Lock check
        if (nowMs >= sessionStartMs - 60 * 60 * 1000) {
            return { ok: false, text: "Lukustatud — eelstaatust ei saa enam muuta.", type: "warning" }
        }
        ...
        if (newStatus === "kinnitatud") {
            ...
            if (kCount >= capacity) {
                return { ok: false, text: "Treening on täis. Kinnitamine ei ole võimalik.", type: "warning" }
            }
            ...
            if (overlappingEntry) {
                ...
                const otherStartTime = otherInst.startTime ?? otherDef?.startTime
                const otherEndTime = otherInst.endTime ?? otherDef?.endTime
                const sportLabel = (otherInst.sport || otherDef?.sport || "").toLowerCase()
                return {
                    ok: false,
    ...
    const handleParentPreStatus = async (instId, playerId, newStatus) => {
        ...
        const validation = validatePreStatusChange({
            instId, playerId, newStatus,
            currentInst, def,
            attendance, rosters, instances, definitions,
            nowMs
        })
        if (!validation.ok) {
            setParentMsg({ text: validation.text, type: validation.type })
            return
        }
        ...
        return (
            <>
                <div style={{ maxWidth: "600px", margin: "0 auto", padding: "20px" }}>
                    <h2 style={{ marginBottom: "var(--spacing-md)" }}>Treeningud</h2>
                    ...
                    {totalVisible === 0 ? (
                        ...
                        <>
                            {(activeSessions.length + todaySessions.length + upcomingSessions.length + pastSessions.length) > 0 && (
                                <h3 style={{ marginTop: 0, marginBottom: "var(--spacing-md)" }}>Minu treeningud</h3>
                            )}
                            <SessionGroup
                                title="Aktiivne"
        ...
        if (lastId) {
            ...
            if (match) {
                ...
                lastSessionBanner = (
                    ...
                    >
                        <div style={{ fontSize: "13px", color: "#3b82f6", fontWeight: "bold", marginBottom: "var(--spacing-xs)" }}>Jätka viimast treeningut</div>
                        <div style={{ fontSize: "15px", fontWeight: "bold" }}>{formatEstonianDate(lInst.date)} — {lTime}</div>
                        <div style={{ fontSize: "13px", color: "var(--color-text-secondary)", textTransform: "capitalize" }}>{lSport}</div>
                    </div>
                )
            }
        }
    ...
    return (
        <div style={{ maxWidth: "600px", margin: "0 auto", padding: "20px" }}>
            <h2 style={{ marginBottom: "var(--spacing-lg)" }}>Treeningud</h2>
